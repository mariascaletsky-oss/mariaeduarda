...
# ---------- PÁGINA 2: CARREGANDO ----------
    ...
    st.write("Aguarde enquanto revelamos seu signo...")
...
# ---------- PÁGINA 3: RESULTADO ----------
    ...
    st.markdown(
        f"<h2 style='color:{cor_texto};text-align:center;'>Olá <b>{nome}</b>, seu aniversário é no dia <b>{dia} de {mes_nome}</b>!</h2>",
        unsafe_allow_html=True
    )
    st.markdown(f"<h2 style='color:{cor_texto};text-align:center;'>Você é de <b>{signo}</b> 🌟</h2>", unsafe_allow_html=True)
    
    # Conselho
    if st.button("Quero um conselho 💡"):
        st.success(frase)
    
    # Famoso
    if st.button("Mostrar famoso do meu signo 🌟"):
